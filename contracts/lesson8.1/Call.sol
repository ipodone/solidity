...
// 3、另外：
// receive() 函数：这是 Solidity 0.6.0+ 引入的专门用于接收纯 ETH 转账的特殊函数
// receive() external payable {
    // 当合约收到纯ETH转账时自动执行
    // 例如：直接向合约地址转账，不附带任何数据
// }
// receive() 和 payable fallback() 同时存在执行优先级：先找匹配的函数名 → 执行；没有匹配 → 检查是否有 receive()（纯转账）；没有 receive() → 执行 fallback()（如果有）
// contract Both {
//     receive() external payable {
//         // 纯ETH转账时执行这个
//     } 
...
//         // 带数据调用时执行这个
//         // 如果是纯转账且没有receive()，也会执行这个
//     }
// }
// 纯 receive() 函数 - 只能接收纯ETH转账（没有数据）
// fallback() 函数 - 接收纯ETH转账（没有数据）、也处理未知函数调用（带数据）；既处理：send(1 ether)、也处理：call{value: 1 ether}("someData")
// 这是智能合约的"银行账户"功能——允许别人给你打钱！

// uint public constant MAX_SUPPLY = 1_000_000; —— _数字分隔符、提高可读性（Solidity 0.7.0+支持）
// 大金额为何3位一组，不太好啊：你说得对！从数学和编程角度，3位一组确实不是最自然的，特别是对中国人来说。这其实是历史和习惯的问题。
// 1、为什么是3位一组？英语国家习惯：
// 英语数字读法：
...
// 中国程序员可能更想要：
// uint public constant MAX_SUPPLY = 100_0000;  // 100万
// 但这样其他国家的开发者看不懂 😅
// 
// 对比表：
// 数字	    英文（3位组）	    中文（4位组）
...
// Token 代币解释: 即Web3 == Token
// 1. Token = 区块链上的数字资产凭证 - 核心特征：数字化、可编程、可转移、可验证
// 2. Token vs Coin 区别
//          Coin（币）	                Token（代币）
// 区块链	有自己的区块链（BTC、ETH）	依赖其他区块链（ERC20在以太坊上）
// 用途	    支付、价值存储	            特定应用场景
// 创建	    需要建立整个区块链	        只需部署智能合约
// 例子	    Bitcoin、Ethereum	        USDT、UNI、AAVE
// 3. Token 标准（ERC标准）- ERC-20 ⭐（最常用）、ERC-721 🖼️（NFT）、ERC-1155 🎮（游戏/多代币）
// 4. 如何创建自己的 Token？简单 ERC20 代币合约：MyToken - 这里就知道为啥叫MyToken了，此处的同质化代币，token都一样，！！！代表一个代币合约；区别与ERC721的NFT非同质化代币，每个tokenId都不一样，!!!此处代表一个NFT
// Token 让一切皆可代币化：从货币到艺术品，从投票权到游戏道具，区块链正在用 Token 重塑数字世界的价值表达方式！

// 授权转账后，被授权人进行授权转账，也要消耗gas，那为何被授权人愿意进行授权转账呢
// 这是一个非常好的问题！触及了 DeFi 经济模型的核心。被授权人（如 Uniswap）确实需要支付 Gas，但他们通过商业模式和激励机制来解决这个问题。
// ========手续费
// 总结回答你的问题：
// 被授权人（如 Uniswap）愿意支付 Gas 是因为：
// 1. 手续费收入覆盖成本：0.3% 手续费远高于单笔 Gas 成本
// 2. 规模效应：海量交易摊薄了平均 Gas 成本
// 3. 用户间接支付：Gas 成本通过手续费、汇率差等形式由用户承担
// 4. 战略价值：即使短期亏损，长期生态价值巨大
// 5. 商业模式设计：批量处理、Layer2、Gas 补贴等技术优化
// 本质上：这不是慈善，而是精心设计的商业模式！用户支付费用，协议提供服务，Gas 只是运营成本的一部分。只要收入 > 成本 + Gas，这个生意就能持续。
// 就像淘宝不会因为要付服务器费用就不做生意一样——成本是业务的一部分，关键是有利润！💰
